...
/* Writes human-readable map coordinates to the given strings, based on the
   given block coordinates. Ranges from (0,0) (85°10'N 85°10'W) to (1023,1023)
   (85°20'S 85°20'E). */
void coordinatestrings(STRPTR lat, STRPTR lon, UWORD x, UWORD y)
{
    ...
    sprintf(lat, "%2d°%2d'%c", degree, minute*10, ns);
    if (x<512)
    {
        we='W';
        x=512-x;
    ...
    sprintf(lon, "%2d°%2d'%c", degree, minute*10, we);
}
...
void newgame()
{
    ...
    for (i=0; i<128; i++)
    {
        ...
        sprintf(getcity(i)->store.name, "«%s»", getcity(i)->name);
